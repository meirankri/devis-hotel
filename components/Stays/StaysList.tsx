...
import { Plus, Edit2, Trash2, Calendar, MapPin, Eye, EyeOff, ExternalLink } from 'lucide-react';
              ...
              <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 group">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {stay.isActive ? <EyeOff className="h-4 w-4 text-white" /> : <Eye className="h-4 w-4 text-white" />}
                        </Button>
                        <Button
                          variant="secondary"
                          size="icon"
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                    ...
                    <span>
                      {format(new Date(stay.startDate), 'd MMM', { locale: fr })} - 
                      {format(new Date(stay.endDate), 'd MMM yyyy', { locale: fr })}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">{t('partialBooking')}:</span>
                      <span className={`font-medium ${stay.allowPartialBooking ? 'text-green-600' : 'text-gray-400'}`}>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-mono text-xs text-indigo-600">/{stay.slug}</span>
                    </div>
                  </div>
                </div>
              </div>
